...
parser.add_argument(
    "--lr", "--learning_rate", default=0.002, type=float, help="initial learning rate"
)
...
def val(net, val_loader, k):
    ...
    if acc > best_acc[k - 1]:
        ...
        save_point = "./checkpoint/%s_net%d.pth.tar" % (checkpoint_name, k)
...
net1.load_state_dict(torch.load("./checkpoint/%s_net1.pth.tar" % checkpoint_name))
net2.load_state_dict(torch.load("./checkpoint/%s_net2.pth.tar" % checkpoint_name))
